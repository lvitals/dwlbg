...
static const char usage[] =
	"Usage: dwlbgctl output NAME [<options>]\n"
	"       dwlbgctl [--help] [--version]\n"
	"\n"
	"Output options:\n"
	...
	"\n"
	"General options:\n"
	"  -V, --version   Show the version of dwlbg\n"
	"  -h, --help      Show this text\n"
	"\n"
	"For the available values for each option, consult dwlbg(5).\n";
...
int main(int argc, char * argv[]) {
	...
	int len = snprintf(remote.sun_path, path_size, "%s/dwlbg", runtime);
	if (path_size <= len) {
	...
	if (connected == -1) {
		perror("Unable to connect to dwlbg socket");
		free(buffer);
		close(sock_fd);
		return 1;
	}
	...
	if (send(sock_fd, buffer, strnlen(buffer, buffer_size - 1), 0) == -1) {
		perror("Unable to send command to dwlbg");
		goto close_err;
	}

	int recv_len = recv(sock_fd, buffer, buffer_size - 1, 0);
	if (recv_len < 0) {
		perror("Unable to read response from dwlbg");
		goto close_err;
	}
	else if (recv_len > 0) {
		buffer[recv_len] = '\0';
