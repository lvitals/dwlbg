...
static char * expand_tilde(const char * path) {
	...
	char * expanded = calloc(length + 1, sizeof(char));
	if (!expanded) {
		perror("Unable to allocate path buffer");
		return NULL;
	}
...
static char * expand_config_path(const char * path) {
	...
	if (!expanded) {
		perror("Unable to allocate config path");
		wordfree(&p);
		return NULL;
	}
...
struct dwlbg_output_config * dwlbg_output_config_create(
	...
	struct dwlbg_output_config * opc = calloc(1, sizeof(struct dwlbg_output_config));
	if (!opc) {
		fprintf(stderr, "Failed to allocate memory for output config\n");
		return false;
	}
	wl_list_init(&opc->link);
	wl_list_insert(dwlbg->output_configs.prev, &opc->link);
...
bool configure_output(
		...
		char * property,
		char * value) {
	struct dwlbg_output_config * output = NULL;

	// Try to find an existing config that matches the name.
	struct dwlbg_output_config * cfg = NULL;
	wl_list_for_each(cfg, &dwlbg->output_configs, link) {
	...
	if (!output) {
		output = dwlbg_output_config_create(dwlbg, output_name);
		if (!output) {
			return false;
		}
	}
...
int load_config_file(struct dwlbg_state * dwlbg, const char * path) {
	...
	int result = load_config(dwlbg, config_file, filename);
	free(expanded_path);
	return result;
}
...
int load_config(struct dwlbg_state * dwlbg, FILE * config_file,
	...
	// square brackets.
	dwlbg_configurator_t * configurator = configure_global;
	...
	while (getline(&line, &n, config_file) > 0) {
		...
		if (entry[0] == '[' && entry[strlen(entry) - 1] == ']') {
			char * saveptr = NULL;
			char * section_type = strtok_r(entry + 1, " ]", &saveptr);
		...
		eq[0] = '\0';
		char * property = trim(entry);
		char * value = trim(eq + 1);

		if (!configurator(dwlbg, section_name, property, value)) {
			fprintf(stderr, "[%s:%d] Failed to parse property '%s'\n",
				filename, lineno, property);
			ret = -1;
			break;
		}
	}
